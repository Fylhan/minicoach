...
class Tools
{
	public static function parserI($int) {
		return intval(trim($int));
	}
	public static function parserF($decimal) {
		return floatval(trim($decimal));
	}

	public static function parserS($str) {
		$res = trim($str);
	...
	public static function deparserS($str) {
		return stripslashes(trim($str));
	}
	public static function parserGeneric($str) {
		$res = trim($str);
		return $res;
	}
	 ...
	 */
	public static function secure($var, $level=0){
		if (is_array($var)) {
		...
		elseif (is_string($var)) {
			...
			elseif ($level==1) {
				...
				$var = str_replace(array('&', '<', '>', '"'), array('&quot;', '&lt;', '&gt;', '&quot;'), $var);//htmlspecialchars
			}
		}
		return $var;
	}
	 ...
	 * @return false sinon
	 */
	public static function isEmailValide($email) {
	...
	public static function approximeMinute($minute) {
	...
	public static function dateFr($timestamp, $heure = true, $le = true) {
		...
		if (date('Y', $timestamp) == date('Y', $timestampCourant)) {
			...
			elseif (date('z', $timestamp) == date('z', $timestampCourant) - 1) { // La veille
				...
				$s_date = ($le ? 'le ' : '') . (date('d', $timestamp) + 0) . ' ' . nomMois(date('n', $timestamp))
						. ($heure ? ' à ' . date('G\hi', $timestamp) : '');
			}
		}
		else { // Une année différente
			$s_date = ($le ? 'le ' : '') . (date('d', $timestamp) + 0) . ' ' . nomMois(date('n', $timestamp)) . date(' Y', $timestamp)
					. ($heure ? ' à ' . date('G\hi', $timestamp) : '');
		}
		return $s_date;
	}
	 ...
	 */
	public static function dateToRFC822($timestamp) {
	 ...
	 */
	public static function afficherDateEvenement($timestamp) {
	...
	public static function getTri() {
		...
		else if (isset($_POST['tri']) && $_POST['tri'] != NULL) {
			$tri = parserUrl($_POST['tri']);
		}
		else if (isset($_COOKIES['tri']) && $_COOKIES['tri'] != NULL) {
			$tri = parserUrl($_COOKIES['tri']);
		}
		else if (isset($_SESSION['tri']) && $_SESSION['tri'] != NULL) {
	 ...
	 * @param string $tri Tri séléctionné
	 * @return int Le tri é effectuer
	 */
	public static function calculSqlTri($tri) {
	 ...
	 * @return int Le tri é effectuer
	 */
	public static function calculUrlTri($tri, $default) {
	 ...
	 */ 
	public static function calculNbElmnt() {
	 ...
	 */ 
	public static function calculNbPage($nbParPage, $nbElmnt) {
	...
	public static function echa($array) {
		echo '<pre>';
		var_dump($array);
		echo '</pre><br />';
	}
	 ...
	 */
	public static function parserUrl($url, $elag = true, $strtolower = true) {
		if ($strtolower) {
	...
	public static function deparserUrl($url) {
		$url = ucwords($url);
		$url = str_replace('-', ' ', $url);
		return $url;
	}
	 ...
	 */
	public static function cleanerUrl($url) {
		return 'www.' . preg_replace('!(?:http://)?(?:www\.)?(.+)!i', '$1', $url);
	}

	// Met au pluriel un mot
	public static function pluriel($a_i_nombreElements, $a_s_mot) {
	...
	public static function ajoutS($nbElmnt, $str) {
		if ($nbElmnt > 1) {
			return $str . 's';
		}
		else {
	 ...
	 */
	public static function isImage($dossierOuChercher, $nomImage) {
	 ...
	 */
	public static function getExtrait($txt, $longueur = 300) {
		if (strlen($txt) > 300)
			$fin = '...';
		return substr($txt, 0, $longueur) . @$fin;
	}

	public static function getMetaTitle($data = '', $page = 1) {
	...
	public static function getMetaDesc($data = '', $page = 1) {
		if (isset($page) && NULL != $page && 0 != $page && 1 != $page) {
			$pagination = ' - Page  ' . $page;
		}
		if ($data == '') {
	...
	public static function getMetaKw($data = '') {
	...
	*/
	...
	public static function getModule() {
		...
		else if (isset($_POST['module']) && $_POST['module'] != NULL) {
			$module = parserUrl($_POST['module'], false, false);
		}
		else if (isset($_COOKIES['module']) && $_COOKIES['module'] != NULL) {
			$module = parserUrl($_COOKIES['module'], false, false);
		}
		else if (isset($_SESSION['module']) && $_SESSION['module'] != NULL) {
	...
	public static function getAction() {
		...
		else if (isset($_COOKIES['action']) && $_COOKIES['action'] != NULL) {
			$action = parserUrl($_COOKIES['action'], false, false);
		}
		else if (isset($_SESSION['action']) && $_SESSION['action'] != NULL) {
	...
	public static function logThatException($e) {
		$data = "\n" . '<h2>' . get_class($e) . ' : ' . date(DATE_RFC1123, time())
				. '</h2><table class="xdebug-error" dir="ltr" border="1" cellspacing="0" cellpadding="1"><code>' . $e->getMessage() . '</code> in ' . $e->getFile()
				. ':' . $e->getLine() . '<br />Stack trace: <pre><code>' . $e->getTraceAsString() . '</code></pre></table><div id="bottom"></div>';
		...
		if (0 == filesize($filename)) {
			$data = '<!DOCTYPE html><html><head>
				...
				<h1>Exception log : <a href="#bottom">Bottom</a></h1>'
					. $data;
		}
		else {
	...
	public static function getUrlImgEmail($email) {
		$emailPath = str_replace(array('@', '.'), array(HAAT, DOHOT), $email);
	...
	public static function creerImgEmail($email) {
		$AT = HAAT;
		$DOT = DOHOT;
		$taille = strlen($email);
	...
	public static function minifyCss($str) {
